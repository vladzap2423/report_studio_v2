...
def load_input(path: str) -> pd.DataFrame:
    ...
    for col in [VID_COL, COL_FIO, COL_SERVICE, COL_FED_CODE, COL_CODE, COL_STATE, COL_SPEC, COL_EXTRA]:
...
def aggregate_for_patient_style(df: pd.DataFrame, meds_map: dict, profile_map: dict):
    df = ensure_optional_columns(
        df,
        [COL_FIO, COL_SERVICE, COL_FED_CODE, COL_STATE, COL_SPEC, COL_EXTRA, COL_DATE],
    )
    ...
    g = df.groupby(
        ["Профиль", COL_FIO, COL_CODE, COL_SERVICE, COL_FED_CODE, COL_STATE, COL_DATE, COL_SPEC, COL_EXTRA],
        as_index=False,
        dropna=False,
    ).agg(
        **{
