...
def build_workbook(data_dict, title_column: str, include_no_nurse: bool) -> Workbook:
    ...
    for service, persons in data_dict.items():
        title = unique_sheet_title(wb, str(service))
...
def main():
    ...
    ap.add_argument("--outdir", required=True)  # <-- важно
    args = ap.parse_args()
    ...
    wb_d.save(os.path.join(args.outdir, "Диспа_Врачи.xlsx"))
    wb_n.save(os.path.join(args.outdir, "Диспа_Медсестры.xlsx"))
