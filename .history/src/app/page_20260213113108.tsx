            ...
            <div ref={scriptsRef} className="mt-12">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-sm text-slate-600">
                  Двойной клик по скрипту запускает отчёт. По правой кнопке — меню с запуском и инструкцией.
                </p>
              </div>
          ...
          <div className="relative z-10 w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-600">
              <div>
                ?????:{" "}
                <span className="font-medium text-slate-900">
                  {scriptUploadFiles.length ? `${scriptUploadFiles.length} ??.` : "?? ???????"}
                </span>
              </div>
