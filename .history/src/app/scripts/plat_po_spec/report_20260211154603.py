...
VID_PROF = "профосмотр"  # сравнение в lower().strip()
...
COL_EXTRA = "Персонал. Дополнительный персонал/ресурсы"
...
def load_input(path: str) -> pd.DataFrame:
    ...
    for col in [VID_COL, COL_PROFILE, COL_STATE, COL_FIO, COL_SERVICE, COL_CODE, COL_SPEC, COL_EXTRA]:
...
def aggregate_for_patient_style(df: pd.DataFrame, medicaments_map: dict) -> pd.DataFrame:
    ...
    for c in [COL_PROFILE, COL_FIO, COL_SPEC, COL_EXTRA, COL_STATE, COL_DATE]:
        if c not in df.columns:
            if c == COL_DATE:
    ...
    g = (
        df.groupby(
            [COL_PROFILE, COL_FIO, COL_CODE, COL_SERVICE, COL_STATE, COL_DATE, COL_SPEC, COL_EXTRA],
            as_index=False,
            dropna=False,
        )
        .agg(**{
    ...
    sort_cols = [COL_PROFILE, "ФИО", "Дата", "Услуга", "Специалист/Ресурс.Выполнение"]
...
def write_patient_like_sheet(ws, df_part: pd.DataFrame, medicaments_map: dict):
    ...
    ws.cell(r, 5, value=total_qty).font = Font(bold=True)

    c6 = ws.cell(r, 6, value=round(total_sum, 2)); c6.font = Font(bold=True); c6.number_format = "0.00"
    c7 = ws.cell(r, 7, value=round(total_meds, 2)); c7.font = Font(bold=True); c7.number_format = "0.00"
    c8 = ws.cell(r, 8, value=round(total_dist, 2)); c8.font = Font(bold=True); c8.number_format = "0.00"
