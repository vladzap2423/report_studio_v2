...
export async function GET(request: Request) {
  try {
    ...
    // Подготовка запроса с явной типизацией
    ...
    // Передаем параметры как tuple [limit, offset]
    const services = stmt.all(limit, offset) as Service[];

    // Подсчет общего количества
    const totalStmt = db.prepare<{ count: number }>(`SELECT COUNT(*) AS count FROM services`);
