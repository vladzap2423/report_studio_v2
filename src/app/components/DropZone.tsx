...
export default function DropZone({ accept = ".csv,.xlsx,.xls", onFilesPicked }: DropZoneProps) {
  ...
  const validateFiles = useCallback((picked: File[]) => {
    ...
    const bad = picked.find(
      (f) => ![".csv", ".xlsx", ".xls"].some((ext) => f.name.toLowerCase().endsWith(ext))
    );

    if (bad) {
      return `Файл "${bad.name}" не поддерживается. Разрешены CSV / XLSX / XLS.`;
    }

    const total = picked.reduce((s, f) => s + f.size, 0);
    if (total > 200 * 1024 * 1024) {
      return "Слишком большой объем. Максимум 200MB.";
    }
        ...
        className={`
          ...
          ${
            isDragging
              ...
              : "border-slate-300 bg-white/40 hover:border-slate-400 hover:shadow-md"
          }
        `}
      >
        <svg className="h-12 w-12 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeWidth={2}
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 12v-8m0 0L8 8m4-4l4 4"
          />
        </svg>

        <p className="text-lg font-semibold text-slate-900">Перетащите файлы сюда</p>
        <p className="text-sm text-slate-600">или выберите с компьютера (CSV / XLSX / XLS)</p>

        <button
          type="button"
          onClick={onPickClick}
          className="mt-3 rounded-full bg-slate-900 px-6 py-2 text-sm font-medium text-white hover:bg-slate-800 active:bg-slate-900"
        >
          Выбрать файл(ы)
        </button>
