    ...
    <div className="p-6 max-w-screen-2xl mx-auto">
      ...
      <div className="mb-5 flex items-center gap-4 flex-wrap">
        <button
            ...
            className={`
                px-4 py-1.5 text-sm font-medium rounded-2xl border transition-all duration-200 ease-in-out
                ${showOnlyUsed
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredServices.map((item) => (
              <tr key={item.ID} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-1.5 text-sm border-r border-gray-200">
                  <div
                    className="line-clamp-2 max-h-10 overflow-hidden"
                    title={item.price_service ?? ""} // ← подсказка при наведении
                  >
                    <input
                ...
                <td className="px-3 py-1.5 text-sm border-r border-gray-200">
                  <div
                    className="line-clamp-2 max-h-10 overflow-hidden"
                    title={item.okmu_service ?? ""} // ← подсказка при наведении
                  >
                    <input
                ...
                <td className="px-3 py-1.5 text-center text-sm border-r border-gray-200">
                  <input
                    ...
                    className="appearance-none w-full mx-auto border border-transparent bg-transparent px-1.5 py-0.5 text-center text-sm rounded focus:border-blue-400 focus:bg-white focus:outline-none transition [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
                </td>
